...
def read(data_dir, train_dir, test_dir):
    ...
    with open(train_dir, 'r') as f:
        for line in f.readlines():
            line = line.strip()
            train_data.append(list(line.split(' ')))
...
# learn probabilities based on partially observed data
def POD_EM_learn(graph_model, train_data, test_data):
    probs_learn = []
    graph_model['strides'] = []
    new_train = []
    lldiffs = []
...
lldiffs = POD_EM_learn(graph_model, train_data, test_data)
mean_lldiff = np.mean(lldiffs)
std_lldiff = np.std(lldiffs)

print('./POD_EM_learn.py <{0}> <{1}> <{2}> <{3}>'.format(data_dir.split('/')[-1], 2, train_dir.split('/')[-1], test_dir.split('/')[-1]))
print('--------------------------------------------')
